...
<xsl:template match="entity">
  ...
  <xsl:variable name="inTransactionForce" select="feature/handler-sql[@id=$handlerId and @force-transaction-mode='true']"/>
...
<xsl:if test="$hasQueryFactory">
    ...
    public <xsl:value-of select="$selectClassName"/>Handler(Database database) <xsl:if test="$hasQueryFactory">throws Exception</xsl:if> {
        ...
        <xsl:if test="not($inTransactionForce)">
            <xsl:if test="$inTransaction and java:bean.Util.containsWordIgnoreCase($sqlQuery,'select') and not($hasQueryFactory)">
                <xsl:if test="not(java:bean.Util.containsWordIgnoreCase($sqlQuery,'delete') or java:bean.Util.containsWordIgnoreCase($sqlQuery,'insert') or java:bean.Util.containsWordIgnoreCase($sqlQuery,'update') or java:bean.Util.containsWordIgnoreCase($sqlQuery,'exec'))">
                    <xsl:message terminate="yes">handler-sql[id='<xsl:value-of select="$handlerId"/>'] - Les requêtes de type select doivent se faire hors transaction !</xsl:message>
                </xsl:if>
            </xsl:if>
            <xsl:if test="not($inTransaction) and  (java:bean.Util.containsWordIgnoreCase($sqlQuery,'delete') or java:bean.Util.containsWordIgnoreCase($sqlQuery,'insert') or java:bean.Util.containsWordIgnoreCase($sqlQuery,'update')) and not($hasQueryFactory)">
                <xsl:message terminate="yes">handler-sql[id='<xsl:value-of select="$handlerId"/>'] - Les requêtes de modification doivent se faire en transaction !</xsl:message>
            </xsl:if>
        </xsl:if>
    <xsl:if test="$hasQueryFactory">
...
<xsl:template match="attributes/name" >
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:for-each select="../../../../properties/field[@structure]">
                    ...
                    <xsl:apply-templates select="../../../structure[@name=$structureName]/properties/field[@name=$fieldName]" mode="select_field_type"/>
                </xsl:for-each>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:variable>
    ...
    <xsl:choose>
        <xsl:when test="string-length($javaSqlTypesConstant)=0">
            addGetter("<xsl:value-of select="."/>", new Getter(<xsl:value-of select="position()"/>));
        </xsl:when>
        <xsl:otherwise>
            addGetter("<xsl:value-of select="."/>", new Getter(<xsl:value-of select="position()"/>, <xsl:value-of select="$javaSqlTypesConstant"/>));
        </xsl:otherwise>
    </xsl:choose>
</xsl:template>
