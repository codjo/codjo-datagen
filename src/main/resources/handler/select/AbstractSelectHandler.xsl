...
<xsl:template match="entity">
  ...
  <xsl:variable name="handlerClasseName" select="java:kernel.Util.handlerClassName(concat('AbstractSelect',$entityClassName))" />
 ...
 */
public abstract class <xsl:value-of select="$handlerClasseName"/>
...
<xsl:template match="properties/field[@type]">
    ...
    <xsl:choose>
        ...
        new Getter&lt;<xsl:value-of select="$entityClassName"/>&gt;(<xsl:value-of select="java:bean.Util.index(.) + $relativePos + 1"/>) {
        </xsl:when>
        <xsl:otherwise>
        new Getter&lt;<xsl:value-of select="$entityClassName"/>&gt;(<xsl:value-of select="java:bean.Util.index(.) + $relativePos + 1"/>,  <xsl:value-of select="$javaSqlTypesConstant"/>) {
        </xsl:otherwise>
    </xsl:choose>
            @Override
